/**
 * HollowPay — Payment Page Archiving API
 *
 * DELETE /api/dashboard/payment-pages/[id]
 */
...
import { archivePaymentPage } from '@/lib/services/payment-page.service';
...
export async function DELETE(
  ...
  try {
    ...
    // 2. Archive
    await archivePaymentPage(projectId, publicId);

    return NextResponse.json({ success: true, message: 'Payment page archived.' });
  } catch (error: unknown) {
    console.error('Failed to archive payment page:', error);
