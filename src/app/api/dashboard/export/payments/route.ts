/**
 * HollowPay — Payments CSV Export API
 *
 * GET /api/dashboard/export/payments — Downloads payment attempts CSV report
 */
...
import { exportPaymentsCsv } from '@/lib/services/export.service';
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const csv = await exportPaymentsCsv(proj[0].id, environment);

    return new NextResponse(csv, {
      status: 200,
      headers: {
        'Content-Type': 'text/csv; charset=utf-8',
        'Content-Disposition': `attachment; filename="hollowpay_payments_${environment}_${new Date().toISOString().slice(0, 10)}.csv"`,
      },
    });
  } catch (error: unknown) {
